...
## Receiving Siacoin using a Ledger device
...
Your address is now ready to use! Send your existing siacoin to this address for safekeeping, or give this address to someone else to securely transfer coins to you.

## Sending Siacoin using a Ledger device

Once you've received siacoin at an address generated by your device, you can construct a transaction that spends those coins.
...
1. You need to specify how to split up the value of the output. Typically, a portion of the output is sent to your intended recipient (such as a friend), another portion is paid to the miner, and the rest is returned to you.
2. The transaction needs to be signed. Each output you spend needs a separate signature. We'll be using the Ledger device to sign the transaction.
...
* Identify which address you want to send siacoin to.
* Choose a fee to pay to the miner.
* Generate a "return address" where you will receive the leftover portion of the output. Refer to [Receiving Siacoin](using-the-sia-ledger-nano-s-app.md#receiving-siacoins-using-a-ledger-device) for how to do this.
